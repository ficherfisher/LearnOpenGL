...
int main() // todo
{
    ...
    //SphereGeometry sphergeometry(1, 20.0, 20.0);
    TorusGeometry torusgeomentry(0.5, 0.2, 48);
    //PlaneGeometry planegeometry(2, 2);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        ourShader.setFloat("tag", mixValue);
        // render container
        ourShader.use();
        glBindVertexArray(torusgeomentry.vao);
        //glDrawElements(GL_TRIANGLES, torusgeomentry.indices.size(), GL_UNSIGNED_INT, 0);
        //glDrawElements(GL_POINTS, sphergeometry.indices.size(), GL_UNSIGNED_INT, 0);
        glDrawElements(GL_LINE_LOOP, torusgeomentry.indices.size(), GL_UNSIGNED_INT, 0);
